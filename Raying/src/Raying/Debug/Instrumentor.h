	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					Raying_Core_Error("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
				}
				InternalEndSession();
			}
			...
			else
			{
				if (Log::GetCoreLogger()) // Edge case: BeginSession() might be before Log::Init()
				{
					Raying_Core_Error("Instrumentor could not open results file '{0}'.", filepath);
				}
			}
		}
...
#define Raying_Profile 0
	...
	// could mark the wrong one in your editor!
		...
		#define Raying_FUNC_SIG __PRETTY_FUNCTION__
	#elif defined(__DMC__) && (__DMC__ >= 0x810)
		#define Raying_FUNC_SIG __PRETTY_FUNCTION__
	#elif defined(__FUNCSIG__)
		#define Raying_FUNC_SIG __FUNCSIG__
	#elif (defined(__INTEL_COMPILER) && (__INTEL_COMPILER >= 600)) || (defined(__IBMCPP__) && (__IBMCPP__ >= 500))
		#define Raying_FUNC_SIG __FUNCTION__
	#elif defined(__BORLANDC__) && (__BORLANDC__ >= 0x550)
		#define Raying_FUNC_SIG __FUNC__
	#elif defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901)
		#define Raying_FUNC_SIG __func__
	#elif defined(__cplusplus) && (__cplusplus >= 201103)
		#define Raying_FUNC_SIG __func__
	#else
		#define Raying_FUNC_SIG "Raying_FUNC_SIG unknown!"
	#endif

	#define Raying_Profile_BEGIN_SESSION(name, filepath) ::Raying::Instrumentor::Get().BeginSession(name, filepath)
	...
	#define Raying_Profile_SCOPE(name)
	#define Raying_Profile_FUNCTION()
#endif